    ...
    with settings(
        ...
        if osver == 'FreeBSD' and ftype >= 10:
            ...
            if getfbindpack == '/usr/local/sbin/named' and bindpidfile == bindpid:
                ...
                print('Please be patient, script install and configure DNS Bind to your servers!!!')
                run('pkg install -y bind910')
                run('echo named_enable="YES" >> /etc/rc.conf')

                if server == env.roledefs['dns'][0]:
                    print('Master DNS is '+successfully+' installed and configured on NS1 '+server+'')
                    ...
                    print('Slave DNS is '+successfully+' installed and configured on NS2 '+server+'')
